...
export default function HistoryPage() {
  ...
  const timeline = buildTimeline(events, sleepSessions, awakeSessions);

  const hasItems = timeline.some((day) => day.items.length > 0);

  return (
    <>
      <Header title="История" showBackButton />

      <main className="mx-auto flex min-h-screen max-w-[430px] flex-col gap-4 bg-slate-950 px-5 pb-6">
        <h2 className="mt-2 text-2xl font-bold text-white">История событий</h2>
        ...
        {!hasItems ? (
          <div className="rounded-[24px] border border-slate-800 bg-slate-900 p-8 text-center">
            <p className="text-slate-400">Пока нет событий</p>
          </div>
        ) : (
